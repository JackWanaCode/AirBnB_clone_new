...
@swag_from('swagger_yaml/places_by_user.yml', methods=['GET'])
@app_views.route('/users/<user_id>/places', methods=['GET'])
def places_by_user(user_id=None):
    """
        places route to handle http method for requested places by city
    """
    user_obj = storage.get('User', user_id)
    ...
    if request.method == 'GET':
        all_places = storage.all('Place')
        user_places = [obj.to_json() for obj in all_places.values()
