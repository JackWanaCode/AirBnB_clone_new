...
app.config['MAIL_USERNAME'] = '': #Your email is here. Make sure you set security in email setting to accept the server send email.
...
def delete_post(place_id):
    r = requests.delete("http://0.0.0.0:8002/api/v1/places/" + place_id)
    if r:
...
def update_post(place_id):
    ...
    if form.validate_on_submit():
        ...
        requests.put("http://0.0.0.0:8002/api/v1/places/" + place_id, data = json.dumps(dic))
        for ame_id in form.amenities.data:
            requests.post("http://0.0.0.0:8002/api/v1/places/" + place_id + "/amenities/" + ame_id)
